...
export default function SinglePost() {
  ...
  useEffect(()=>{
    const GetLike = async(e)=>{
      try{
        ...
        await axios.get("http://localhost:5000/user/"+ourUser.id+"/blog/"+Id).then((response)=>{
  ...
  const handleUpdate=async(e)=>{
    ...
    try{
      await axios.patch("http://localhost:5000/user/blog/"+blog.id,data,{
        headers:({
          Authorization:'Bearer '+token
        })
      })
  ...
  const handleLike=async(e)=>{
    ...
    try{
      await axios.post("http://localhost:5000/user/blog/"+Id+"/blog_like",data,{
        headers:({
          Authorization:'Bearer '+token
        })
      }).then((response)=>{
  ...
  const handleUnLike=async(e)=>{
    if(window.confirm("Are you sure you want to unlike this blog")){
      try{
        await axios.delete("http://localhost:5000/user/blog/"+Id+"/blog_like",{
          headers:({
            Authorization:'Bearer '+token
          })
        }).then((response)=>{
